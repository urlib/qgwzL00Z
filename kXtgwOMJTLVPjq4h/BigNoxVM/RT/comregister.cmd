...
REM correct NoxVMSVC and register the right VBoxC.dll.
REM
...
:register_x86
@echo on
"%_VBOX_DIR%NoxVMSVC.exe" /UnregServer
regsvr32 /s /u "%_VBOX_DIR%NoxVMC.dll"
if %fdontuninstallproxy% == 0 %windir%\system32\regsvr32 /s /u "%_VBOX_DIR%VBoxProxyStub.dll"
@if %fUninstallOnly% == 1 goto end
"%_VBOX_DIR%NoxVMSVC.exe" /RegServer
regsvr32 /s    "%_VBOX_DIR%NoxVMC.dll"
...
@echo on
"%_VBOX_DIR%NoxVMSVC.exe" /UnregServer
%windir%\system32\regsvr32 /s /u "%_VBOX_DIR%NoxVMC.dll"
%windir%\syswow64\regsvr32 /s /u "%_VBOX_DIR%x86\NoxVMC-x86.dll"
if %fdontuninstallproxy% == 0 %windir%\system32\regsvr32 /s /u "%_VBOX_DIR%%s64BitProxyStub%"
if %fdontuninstallproxy% == 0 %windir%\syswow64\regsvr32 /s /u "%_VBOX_DIR%x86\VBoxProxyStub-x86.dll"
if %fUninstallOnly% == 1 goto end
"%_VBOX_DIR%NoxVMSVC.exe" /RegServer
%windir%\system32\regsvr32 /s    "%_VBOX_DIR%NoxVMC.dll"
%windir%\syswow64\regsvr32 /s    "%_VBOX_DIR%x86\NoxVMC-x86.dll"
if %fNoProxy% == 1 goto end
